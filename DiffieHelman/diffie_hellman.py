...
def find_generator(p):
    ...
    Для алгоритма используется поле вычетов по модулю
    p, где  p — большое простое число.

    1. Выбор простого числа p
    2. Вычисление делителей p − 1
    3. Перебор кандидатов для g
    Выбираем g из диапазона 2 <= g < p
    Для каждого делителя q числа p - 1 проверяем условие g^((p - 1)/q) != 1 mod p
    4. Подтверждение, что g является примитивным корнем
    - Примитивный корень в математике — это такое число g,
    которое является порождающим элементом мультипликативной группы вычетов по модулю p
    - Мультипликативная группа вычетов по модулю p — это множество всех целых чисел
    от 1 до p−1, которые взаимно просты с p, с операцией умножения по модулю p
    - Число g является порождающим элементом Мультипликативной группы
    если для любого элемента из Мультипликативной группы существует такое k, что
    a == g^k mod p где 1 <= g <= p-1
    """
    if not isprime(p):
