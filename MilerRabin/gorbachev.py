...
print(codes)
encr_word = [pow(m,e,n) for m in codes]
print(f'encrypted is {encr_word}')
decr_word = [pow(c,d,n) for c in encr_word]
print(f'decrypted is {decr_word}')
print(''.join([chr(char+64) for char in decr_word]))

start_key = 2
while (pow(encr_word[0], start_key, n) != decr_word[0]):
    start_key+=1
print(start_key)
